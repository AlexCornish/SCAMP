...
def writeToCSV(fileName,data):
    # - fileName: (String) The name of the original csv file from which the name for the .csv file will be made.
    # - data: (Dataframe) The formatted data in a pandas dataframe.
    # removes the .csv extension from the filename and adds the .csv extension.
    newFileName = str(input(" Enter what you'd like to name the output of this program. (DON'T ADD .CSV)"))
    tempName = newFileName + ".csv"
    # converts the dataframe to a .csv file and removes the indexes from the dataframe.
    data.to_csv(os.path.join(fileName,tempName),index=False)
...
def createCustomFormattedDataFrame(dataFrame):
    ...
    if labelAdd == 1:
        ...
        newGroupFrame = changeRowHeaders(readCSV(newGroupPath)).drop([0])
        newDataFrame = changeRowHeaders(readCSV(newPath)).drop([0])
...
def wpProcessing():
    ...
    writeToCSV(os.path.join(path,'RawData',"Commodity"),createCustomFormattedDataFrame(changeRowHeaders(readCSV(newPath)).drop([0])))
